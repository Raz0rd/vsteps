...
# -*- coding: utf-8 -*-
"""
Insere CPFs do estado PA usando AMPA_RRAP_ACROx.txt + API externa.
Formato arquivo: CPF:DD/MM/YYYY
Processa em chunks para arquivo grande (2M+ linhas)
"""
...
CHUNK_SIZE = 50000  # Processa 50k CPFs por chunk
TARGET_INSERT = 10000  # Meta de inserções
...
def main():
    print(f"\n{B}{'='*60}")
    print(f"  Popular Fila — CPFs do estado PA via AMPA_RRAP_ACROx.txt")
    print(f"  DDD: {DDD}")
    print(f"  Threads: {THREADS}")
    print(f"  Chunk: {CHUNK_SIZE:,} CPFs")
    print(f"  Meta: {TARGET_INSERT:,} inserções")
    ...
    with open(LISTA_FILE, "r", encoding="utf-8") as f:
        ...
        for linha in f:
            ...
            if len(chunk_cpfs) >= CHUNK_SIZE or inserted >= TARGET_INSERT:
                chunk_num += 1
                print(f"\n{C}Chunk {chunk_num}: {len(chunk_cpfs):,} CPFs para processar{W}")
                
                with ThreadPoolExecutor(max_workers=THREADS) as executor:
                    futures = {executor.submit(processar_cpf, cpf_data, existentes): cpf_data for cpf_data in chunk_cpfs}
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            if record:
                                ...
                                if len(batch) >= BATCH_INSERT:
                                    ...
                                    inserted += ok
                                    print(f"  {G}[{UF}]{W} {inserted:,}/{TARGET_INSERT:,} inseridos | uf_err={skipped_uf} | inv={skipped_inv}")
                                    batch = []
                            else:
                                skipped_uf += 1
                        except Exception as e:
                        ...
                        # Progresso
                        if tentativas % 100 == 0:
                            print(f"  {D}[{UF}]{W} Processados: {tentativas:,} | Inseridos: {inserted:,}", end="\r")
                
                chunk_cpfs = []
                
                # Parar se atingiu meta
                if inserted >= TARGET_INSERT:
                    print(f"\n{G}Meta de {TARGET_INSERT:,} inserções atingida!{W}")
                    break
